...
#pragma omp single
			{
				sprintf_s(myFileName, "ShallowWaterEquation_%d.csv",i);//visualize by paraview
	...
	file.close();//</ finish writting>

	wtime = omp_get_wtime() - wtime;
	cout << "total time: " << wtime << endl;
	cout << "each time step cost: " << (wtime / N_t) << " s" << endl;
	cout << "total threads used: " << N_Threads << " threads" << endl;
...
void rk4(double ***k, double ***h_vx_vy){
	//ibc and jbc are two arrays that can satisfy the 
	// the boundary condition at any position of the grid
	for (int i = 0; i < N_x; i++){
